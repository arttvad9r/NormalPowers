...
def _skills_dir() -> str:
    """Locate the NormalPowers skills tree for supported Hermes install layouts."""
    ...
    for candidate in candidates:
        bootstrap_skill = os.path.join(candidate, "using-normalpowers", "SKILL.md")
        if os.path.isfile(bootstrap_skill):
            return candidate

    raise RuntimeError(
        "normalpowers plugin: cannot find the skills/ tree "
        f"(looked at {candidates}). Reinstall the plugin from its repository."
    )
